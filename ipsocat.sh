...
#	Author:	Rat's
#	Dscription: Socat一键脚本-四川骏鑫科技-Jerry
#	Version: 1.0
#	Blog: https://www.moerats.com
#	手机号：13981816463 18908200527 微信：JerryHe0527
# ====================================================
...
config_socat(){
    echo -e "${Green}四川骏鑫科技提示请输入Socat配置信息！${Font}"
    read -p "四川骏鑫科技提示请输入本地端口:" port1
    read -p "四川骏鑫科技提示请输入远程端口:" port2
    read -p "四川骏鑫科技提示请输入远程IP:" socatip
}

start_socat(){
    echo -e "${Green}四川骏鑫科技提示正在配置Socat...${Font}"
	nohup firewall-cmd --add-port=${port1}/tcp >> /root/socat.log 2>&1 &
    ...
    if [ "${OS}" == 'CentOS' ];then
        ...
        echo "nohup firewall-cmd --add-port=${port1}/tcp >> /root/socat.log 2>&1 &
		nohup socat TCP4-LISTEN:${port1},reuseaddr,fork TCP4:${socatip}:${port2} >> /root/socat.log 2>&1 &
        nohup socat -T 600 UDP4-LISTEN:${port1},reuseaddr,fork UDP4:${socatip}:${port2}  >> /root/socat.log 2>&1 &
        " >> /etc/rc.d/rc.local
...
systemctl start rc-local >/dev/null 2>&1
    ...
    echo -e "${Green}有问题联系手机号：13981816463 18908200527 微信：JerryHe0527 ${Font}"
...
install_socat(){
    echo -e "${Green}四川骏鑫科技提示即将安装Socat...${Font}"
    if [ "${OS}" == 'CentOS' ];then
        yum install -y socat
    else
        apt-get -y update
    ...
    echo -e "${Green}四川骏鑫科技提示Socat安装完成！${Font}"
    echo -e "${Green}有问题联系手机号：13981816463 18908200527 微信：JerryHe0527 ${Font}"
    fi
}
